...
const buildTools = (ops: Operations, ctx: ToolContext): McpTool[] => [
  ...
  },
  {
    ...
    description:
      "Update fields on an existing node (position, name, description, detail, icon, colors, border, font, dimensions, autoSize, plus type:'icon'-only color/strokeWidth/alt, type:'linkflow'-only target). `type` can flip a node between any of the 19 hand-authorable variants (rectangle/ellipse/sticky/text/database/server/user/queue/cloud/diamond/hexagon/triangle/parallelogram/document/image/html/icon/component/linkflow — freehand / line / group / table are drawn in the studio); the post-merge schema reparse gates required fields on the new type (image.data.path, icon.data.icon). Setting detail (every node) or html (type:'html') writes the content to <project>/nodes/<id>/{detail.md|view.html}; the file:// ref on the node persists. Empty-string detail empties the file but keeps the ref. Setting target (type:'linkflow') with `{ project, flow }` links the node to another flow; passing `target: null` clears the link back to the unlinked state.",
