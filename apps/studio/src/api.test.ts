...
describe('POST /api/projects', () => {
  it('scaffolds a fresh project (seeflow.json + flows/main/flow.json) at the supplied path', async () => {
    ...
    expect(entry.flowPath).toBe('flows/main/flow.json');
    expect(entry.projectSlug).toBe('fresh-project');
    expect(entry.flowSlug).toBe('main');
    expect(entry.isDefault).toBe(true);

    const manifest = JSON.parse(readFileSync(join(projectPath, 'seeflow.json'), 'utf-8'));
    ...
    const written = JSON.parse(
      readFileSync(join(projectPath, 'flows', 'main', 'flow.json'), 'utf-8'),
    );
  ...
  it('persists description into seeflow.json and the registry entry when supplied', async () => {
    ...
    // flow.json envelope itself stays description-free — description lives at
    // the project (manifest) level for manifest-driven projects.
    const written = JSON.parse(
      readFileSync(join(projectPath, 'flows', 'main', 'flow.json'), 'utf-8'),
    );
  ...
  it('returns 409 when the target already has a seeflow.json', async () => {
    ...
    writeFileSync(join(projectPath, 'seeflow.json'), JSON.stringify(existingManifest));
    const beforeBytes = readFileSync(join(projectPath, 'seeflow.json'), 'utf-8');
    ...
    // Existing manifest is untouched and not registered.
    expect(readFileSync(join(projectPath, 'seeflow.json'), 'utf-8')).toBe(beforeBytes);
    expect(registry.list()).toHaveLength(0);
  });

  it('returns 409 when the target has a legacy single-flow flow.json', async () => {
    ...
    expect(res.status).toBe(409);
    expect(existsSync(join(projectPath, 'seeflow.json'))).toBe(false);
    expect(registry.list()).toHaveLength(0);
  });
...
// Lightweight fake controller for /api/share/* routes. Tests script the
// behavior they need (resolve/reject + state transitions) so the routes can
// be exercised without booting a real ShareController, fetch mock, or
// WebSocket fake.
interface FakeShare extends ShareController {
  setState: (next: ShareState) => void;
}
const createFakeShare = (init?: {
  ...
  return {
    start: init?.start ?? (async () => ({ url: 'https://share/x', sessionId: 'sid' })),
...
const buildAppWithShare = (share: ShareController) => {
  const registry = createRegistry({ path: tmpRegistry() });
  const app = createApp({
    mode: 'prod',
    staticRoot: './dist/web',
...
describe('POST /api/share/start', () => {
  it('returns 200 { url, sessionId } on happy path', async () => {
    const share = createFakeShare({
      start: async () => ({ url: 'https://share.seeflow.dev/abc', sessionId: 'sess-1' }),
    });
    const { app } = buildAppWithShare(share);
    const res = await app.request('/api/share/start', { method: 'POST' });
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ url: 'https://share.seeflow.dev/abc', sessionId: 'sess-1' });
  });

  it('returns 500 when controller rejects with a generic error', async () => {
    const share = createFakeShare({
      start: async () => {
        throw new Error('share-relay-http-500');
      },
    });
    const { app } = buildAppWithShare(share);
    const res = await app.request('/api/share/start', { method: 'POST' });
    ...
    const body = (await res.json()) as { error: string };
    expect(body.error).toBe('share-relay-http-500');
  });

  it('returns 409 when controller rejects with share-already-active', async () => {
    const share = createFakeShare({
      start: async () => {
...
describe('POST /api/share/stop', () => {
  it('returns 204 on success', async () => {
    const share = createFakeShare();
    const { app } = buildAppWithShare(share);
    const res = await app.request('/api/share/stop', { method: 'POST' });
...
describe('POST /api/share/kick', () => {
  const kick = (app: ReturnType<typeof buildAppWithShare>['app'], body: unknown) =>
    app.request('/api/share/kick', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(body),
    });
  ...
  it('returns 204 on success', async () => {
    const share = createFakeShare();
    const { app } = buildAppWithShare(share);
    const res = await kick(app, { peerId: 'peer-1' });
    expect(res.status).toBe(204);
  });

  it('returns 400 when body is missing peerId', async () => {
    const share = createFakeShare();
    const { app } = buildAppWithShare(share);
    const res = await kick(app, {});
    expect(res.status).toBe(400);
  });

  it('returns 409 when controller rejects with share-not-active', async () => {
    const share = createFakeShare({
      kick: async () => {
  ...
  it('returns 404 when controller rejects with share-peer-not-found', async () => {
    const share = createFakeShare({
      kick: async () => {
...
describe('POST /api/share/rotate', () => {
  it('returns 200 { url } on success', async () => {
    const share = createFakeShare({
      rotateUrl: async () => ({ url: 'https://share.seeflow.dev/new' }),
    });
    const { app } = buildAppWithShare(share);
    const res = await app.request('/api/share/rotate', { method: 'POST' });
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ url: 'https://share.seeflow.dev/new' });
  });
...
describe('GET /api/share/state', () => {
  // Read SSE chunks until `until(buf)` returns true OR the timeout expires.
  ...
  it('opens the stream and emits the initial state frame', async () => {
    const share = createFakeShare({ initialState: { status: 'idle' } });
    const { app } = buildAppWithShare(share);
    const res = await app.request('/api/share/state');
    expect(res.status).toBe(200);
    expect(res.headers.get('content-type')).toContain('text/event-stream');
    if (!res.body) throw new Error('SSE body missing');

    const text = await readUntil(res.body, (s) => s.includes('event: state'));
    expect(text).toContain('event: state');
    expect(text).toContain('"status":"idle"');
  });

  it('emits a new state frame after a controller transition', async () => {
    const share = createFakeShare({ initialState: { status: 'idle' } });
    const { app } = buildAppWithShare(share);
    const res = await app.request('/api/share/state');
    if (!res.body) throw new Error('SSE body missing');

    // Drive a transition after the stream is open — the subscribe callback
    // pushes the new state into the SSE queue. Schedule via setTimeout so
    // the initial frame has been queued before the next state flips in.
    setTimeout(() => {
      share.setState({
        status: 'active',
        sessionId: 'sess-2',
    ...
    const text = await readUntil(res.body, (s) => s.includes('"status":"active"'));
