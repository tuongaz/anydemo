...
describe('seeflow export', () => {
  it('is a registered subcommand and --dry-run bundles without a network call', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'seeflow-export-'));
    writeFileSync(
      join(dir, 'flow.json'),
      JSON.stringify({ version: 2, name: 'Exported', nodes: [], connectors: [] }),
    );
    ...
    expect(out.files).toContain('flow.json');
  }, 20_000);

  it('export without --dry-run fails clearly when not logged in', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'seeflow-export-nologin-'));
    writeFileSync(
      join(dir, 'flow.json'),
      JSON.stringify({ version: 2, name: 'N', nodes: [], connectors: [] }),
    );
    // Point the credential store at an empty dir so no token is found.
    const emptyHome = mkdtempSync(join(tmpdir(), 'seeflow-export-home-'));
    const r = await runCli(['export', dir, '--endpoint', 'https://nope.example'], {
      SEEFLOW_WORKSPACE: dir,
      XDG_CONFIG_HOME: emptyHome,
    });
    expect(r.code).not.toBe(0);
    expect(r.stderr.toLowerCase()).toContain('login');
  }, 20_000);
});
