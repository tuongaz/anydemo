...
- **CLI subcommand dispatch** (`src/cli.ts`): the long `if (sub === '…') { … } else if …` chain near line 199 is the registration point. Each branch is paired with a `runFoo()` helper at the end of the file. Heavy modules (e.g. `./icons/*`) load lazily via `await import(...)` inside the runner — matches `runSchema`, `runE2e`, etc.
...
- **Vendor types are NOT shared:** `IconVendor` in `packages/canvas/src/lib/icon-id.ts` carries 5 vendors (lucide, aws, gcp, azure, iconify) because the canvas resolves all of them. `IconVendor` in `apps/studio/src/icons/paths.ts` carries 3 (aws, gcp, azure) because only those have caches. Keep them distinct — do NOT cross-import.
