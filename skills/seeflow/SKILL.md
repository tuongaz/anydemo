...
| `$projectSlug` | **Captured from the `projects:create` response** (`data.slug` / the `projectSlug` it returns) and authoritative for every follow-up `--project` call. The studio derives it as `slugify(--name)` (e.g. `--name "Order Pipeline"` → `order-pipeline`) — **not** from the planner's `slug` field, which may differ. Never reuse the planner's `slug` as `--project`; always reassign `$projectSlug` from the response (see `phases/p3-scaffold.md` § 1). |
