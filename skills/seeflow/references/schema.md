...
node lives in `<project>/.seeflow/nodes/<nodeId>/`:
...
prepends `.seeflow/nodes/<nodeId>/` and rejects any path that escapes
...
forward-compat; the path must be relative under `.seeflow/`, no leading
`/`, no `..`. Missing files render as a `[seeflow: missing file '…']`
placeholder card.
...
- The envelope's optional reset action exists only when the app has a
  ...
  `scriptPath` is currently anchored at `.seeflow/` for backwards
  compat; per-node anchor is a deferred follow-up.
...
to `<project>/.seeflow/nodes/<id>/view.html` and stores a `file://` ref
...
`<project>/.seeflow/nodes/<id>/view.html` directly — saves trigger a
live reload.
...
(`<project>/.seeflow/nodes/<id>/<filename>`); the studio's per-node
...
- **Path anchor** for `scriptPath` is the node folder
  (`.seeflow/nodes/<nodeId>/`). No leading slash, no `..`. The reset
  action stays anchored at `.seeflow/` until a follow-up.
