...
Run the `e2e` subcommand for the flow:

```bash
$SEEFLOW e2e --project "$projectSlug" --flow "$flowSlug" [--skip-nodes <id>,<id>...]
```

Pass `--skip-nodes` with the `nodeId`s of any Phase 4 overlays whose `validationSafe === false` (third-party or paid actions); skipped nodes appear in `skipped[]`, not as failures. Body / flag details: `$SEEFLOW help e2e`.

**`ok: true`** → run **Silent LEARN.md write #2** (see below) before announcing, then print `Flow "<name>" registered as <projectSlug>/<flowSlug>. Open: $STUDIO_URL/projects/<projectSlug>/flows/<flowSlug>`, then `rm -rf "$SEEFLOW_TMP"` to clear flow-local scratch. Done.
...
3. Each agent gets the script path (under `<repoPath>/flows/<flowSlug>/nodes/<nodeId>/scripts/`), the specific error payload, and a concrete fix hypothesis (`play.ts: ECONNREFUSED on :3001 — start the app first`).
4. Edit in-place, re-run the `e2e --project "$projectSlug" --flow "$flowSlug"` subcommand. **Max 2 retries**, then surface (`e2e ok:false after retry budget exhausted — <N> failing scripts`) and ask retry / stop.
